...
int *makeMore(float *C,float *W1,float *B1,float *W2,float *B2,int count,
                ...
                float *running_var,int lenVocab,int numTokens){
    ...
    double *dprobs;
    unsigned int *n = (unsigned int *)calloc(lenVocab,sizeof(int));
    float *emb=(float *)calloc(INNODES,sizeof(float));
    int *out=(int *)calloc(numTokens,sizeof(int));
    int p=0;
    int numBigrams=1;
    int X[BLOCKSIZE];
    ...
    for(int i=0;i<count;i++){
        ...
        logits=multAB(Hn,W2,numBigrams,HIDNODES,HIDNODES,lenVocab);
        // add bias to the logits
        addBias(logits,B2,numBigrams,lenVocab);
        // get the probabilities
        probs=softMax(logits,numBigrams,lenVocab);
        dprobs=probs2d(probs,lenVocab);
        gsl_ran_multinomial(pr,lenVocab,1,dprobs,n);
        // find which element of n has a 1
        for(int j=0;j<lenVocab;j++){
            if(n[j]!=0) ix=j;
        }
        out[p++]=ix;
